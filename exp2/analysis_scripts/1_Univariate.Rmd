...
-   Part 8: Early visual regions' responses to visually novel information (asks the question of whether we could have observed effects in these regions, in principle, since each pair of VOE events differs across different visual features)
...
ggplot(focal_modelsummaries %>%
         ...
         filter(effect == "event:domain",
                ROI == "supramarginal_L"), aes(extracted_run_number, B_standardized, colour = top_n_voxels)) +
  geom_point() +
  geom_line(aes(group = top_n_voxels)) +
  ylab("Standardized Beta") +
  facet_wrap(~effect+ROI) +
  # facet_wrap(~ effect + factor(ROI, levels = c("supramarginal_L", "supramarginal_R", "superiortemporal_L", "superiortemporal_R", "antParietal_bilateral", "precentral_inferiorfrontal_R", "V1_bilateral", "MT_bilateral")), nrow = 1) +
...
# Part 8: Early visual regions' detection of visual novelty
